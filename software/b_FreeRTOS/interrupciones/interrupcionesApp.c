...
int main ( void ){
    ...
    xTaskCreate  ( taskISRHandler,          /* The function that implements the task. */
                 ( const char * ) "IRS",    /* Text name for the task, provided to assist debugging only. */
                 configMINIMAL_STACK_SIZE,  /* The stack allocated to the task. */
                 NULL,                      /* The task parameter is not used, so set to NULL. */
                 tskIDLE_PRIORITY,          /* The task runs at the idle priority. */
                 &xTaskISRHandler );
...
void GpioIsr(void *InstancePtr)
{
    ...
    // xSemaphoreGiveFromISR() setea *pxHigherPriorityTaskWoken a pdTRUE, porque de esta forma se desbloquea la tarea taskISRHandler() al darle el semaforo desde la interrupcion, permitiendo un cambio de contexto para que luego de la interrupción se ejecute dicha tarea y no la tarea que se estaba ejecutando previamente. 

    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
    // portYIELD_FROM_ISR(xHigherPriorityTaskWoken) fuerza al SO a realizar el cambio de contexto, solo si xHigherPriorityTaskWoken es pdTRUE (fué puesto en pdTRUE por xSemaphoreGiveFromISR( semHandler, &xHigherPriorityTaskWoken)).
